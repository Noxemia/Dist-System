...
try:
    ...
    def leader_election():
        ...
        for vessel_id, success in res.items():
            if success and int(vessel_id) > int(new_leader):
    ...
    def client_add_received():
        ...
        try:
            ...
            if int(my_leader) != node_id: # if this is not the leader forward the request to the leader
                ...
                thread.daemon = True
                thread.start()
                return app.HTTPResponse(status = 201, body=json.dumps({'hello': 'no'}))
            else:
                ...
                thread.daemon = True
                thread.start()
                return app.HTTPResponse(status = 201, body=json.dumps({'hello': 'world'}))
        except Exception as e:
            print (e)
